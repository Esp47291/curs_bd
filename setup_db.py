...
def create_database():
    ...
    cursor.execute('DROP DATABASE IF EXISTS hh_vacancies_database;')
    cursor.execute("CREATE DATABASE hh_vacancies_database;")
    cursor.close()
    conn.close()
...
def create_tables():
    ...
    conn = psycopg2.connect(dbname='hh_vacancies_database', user='postgres', password="simplepassword123", host='localhost')
    cursor = conn.cursor()
...
def add_company(conn, name, description, vacancies_url):
    """Функция отвечающая за создание компаний"""
    cursor = conn.cursor()
    cursor.execute("""
...
def add_vacancy(conn, title, salary, url, company_id):
    """Функция отвечащая за создание вакансий"""
    cursor = conn.cursor()
    cursor.execute("""
